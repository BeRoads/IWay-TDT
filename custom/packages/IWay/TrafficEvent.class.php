...
class IWayTrafficEvent extends AResource{
	...
	if($this->region == 'all'){
		...
		foreach($regions as $region){
			...
			$element_all = $c->get("traffic" . $this->region . $this->lang);
				...
				$element_all = $this->geocodeData($element_all);
		                $c->set("traffic" . $this->region . $this->lang, $element_all);
			}else{
				if(count($element_all->item)>0 && time() - $element_all->item[count($element_all->item)-1]->time > Config::$CACHE_TIME){ 
					$data = $this->getData();
					...
					$merge_element = $this->geocodeData($merge_element);
			                ...
			                $c->set("traffic" . $this->region . $this->lang, $current_element);
				}
			}
	  ...
	  if(is_null($element)){
			...
			$c->set("traffic" . $this->region . $this->lang, $element);
	  }else{
			
			if(time() - $element->item[count($element)-1]->time > Config::$CACHE_TIME){ 
				$data = $this->getData();
	                        $current_element = $c->get("traffic" . $this->region . $this->lang);
	                        $new_element = $this->parseData($data);
	                        $merge_element = new stdClass();
                        	...
                        	foreach($new_element->item as $new_item){
                                	$to_merge = true;
	                                foreach($current_element->item as $current_item){
						echo $new_item->location." == ".$current_item->location." ? ";
	...
	return utf8_encode(TDT::HttpRequest($scrapeUrl)->data);	 
     ...
     private function parseTime($str){
     	  $months = array("janv"=>1, "févr"=>2, "mar"=>3, "avr"=>4, "mai"=>5, "juin"=>6, "juil"=>7, "aout"=>8, "sept"=>9, "oct"=>10, "nov"=>11, "déc"=>12);
     	  switch($this->region){
     	  
     	  		case "wallonia" : 
     	  				//sam., 10 sept. 2011 23:57:43 +0200
     	  			  preg_match("/(\w+)., (\d+) ([a-zA-Zé]+). (\d+) ((\d\d):(\d\d):(\d\d)) +(\d+)?/",$str,$match);
     ...
     public static function getDoc(){
          return "TrafficEvent return the latest traffic events by region or around a geographic coordinate.";
     }
